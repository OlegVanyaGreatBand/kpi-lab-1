...
func TestSimpleTestFactory(t *testing.T) {
	...
	ctx.RegisterModuleType("go_tested_binary", SimpleTestFactory)
	...
	if err := ctx.WriteBuildFile(buffer); err != nil {
		...
		if !strings.Contains(text, "default out/bin/test-out") {
			t.Errorf("Generated ninja file does not have default out/bin/test-out statement")
		}
	}
}


func TestSimpleTestFactoryOptional(t *testing.T) {
	ctx := blueprint.NewContext()
